![Banner](banner.png)

# Hi, I'm Lakshmi Priya 👋

### Business Analyst | Data Analytics | Project Management | Data Storytelling

I am a Business Analyst with 4+ years of professional experience in Sales, Marketing, and Market Research, currently transitioning into Business Analytics.

Passionate about transforming business problems into data-driven solutions through analytics, visualization, and strategic decision-making.

## About Me

🎓 MBA in Sales & Marketing
🎓 MS in Business Analytics
📊 Passionate about solving business problems using data
📈 Experienced in Business Analysis, Data Analytics, and Dashboard Development
📋 Knowledge of Agile, Scrum, SDLC, Requirement Gathering & Project Management
💼 Open to Business Analyst Opportunities

## Technical Skills

- SQL
- Power BI
- Microsoft Excel
- Tableau
- Power Query
- DAX
- Data Cleaning
- Data Visualization
- Business Analysis
- KPI Development
- Dashboard Design
- JIRA
  
## Domain Experience

- Supply Chain Analytics
- Healthcare Analytics
- Marketing Analytics
- Sales Analytics

## Featured Projects

### Supply Chain Performance Analytics & Business Intelligence

Developed an end-to-end analytics solution using Excel, SQL, and Power BI to evaluate vendor performance, procurement, inventory, logistics, and returns.

---

### Healthcare Analytics Dashboard

Designed an interactive Power BI dashboard to monitor admissions, revenue, readmission rates, and insurance claim losses.

---

### Marketing Performance Analytics

Analyzed Google Analytics data to evaluate campaign performance, customer engagement, and marketing ROI using Excel and Power BI.

## Core Competencies

✔ Requirement Gathering

✔ Business Process Analysis

✔ Data Cleaning

✔ SQL Query Writing

✔ Dashboard Development

✔ KPI Design

✔ Business Storytelling

✔ Stakeholder Management

✔ User Story Creation

✔ BRD & FRD Documentation

✔ Data-Driven Decision Making

## Connect With Me

LinkedIn:
[www.linkedin.com/in/lakshmi-priya](https://www.linkedin.com/in/lakshmi-priya-115b6b1a3/)

Email:
lakshmipriya2762@email.com

